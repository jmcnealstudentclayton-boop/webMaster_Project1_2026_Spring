...
function getSortParams() {
    ...
    const column = col === 'year' ? 'release_year' : col === 'runtime' ? 'runtime' : 'title';
...
async function loadMovies() {
    ...
    try {
        ...
        grid.innerHTML = data.map(m => `
            <a href="detail.html?id=${m.movie_id}" class="bg-slate-800 border border-slate-700 rounded-lg overflow-hidden hover:border-indigo-500 transition-colors block no-underline text-inherit card-lift animate-fade-in">
                ${m.poster_url
                ...
                <div class="p-5">
                    ...
                    <p class="text-slate-400 text-sm">${esc(String(m.release_year ?? ''))} &middot; ${m.runtime ?? '?'} min</p>
                    <p class="mt-2 text-sm text-slate-300">${esc((m.description ?? '').substring(0, 150))}${(m.description?.length ?? 0) > 150 ? '…' : ''}</p>
                </div>
            </a>
        `).join('');
